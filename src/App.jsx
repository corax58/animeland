...
const App = () => {
  ...
  const handleSearch= async (query)=>{
    ...
    const temp = await axios.get("https://api.jikan.moe/v4/anime?q="+query).then(res=>res.data.data)
  ...
  >
    <GridItem pl='2' area={'header'}>
     ...
     <form style={{width:"100% ", marginRight:'10'}} onSubmit={(event)=>{
      event.preventDefault();
      if(ref) handleSearch(ref.current.value);

    }}><Input ref={ref} m={'3'} borderRadius={'20'} placeholder='Search anime...' variant={"filled"}  style={{width:'80%' , marginLeft:'7%', marginRight: 'auto'}}></Input></form>
